...
| `lib/api/providers/` | Provider-based anime sources: Anidb, Anipub, Anineko, Animepahe — registered in `ProviderRegistry` (AllAnime removed 2026-08, dead; Animepahe kept but removed from default stack, Cloudflare-walled). Anidb uses a curl subprocess on desktop (CF blocks Dart's BoringSSL JA3); mobile falls back to dart:io http |
